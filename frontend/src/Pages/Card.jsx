...
export default function Card({ imagen, titulo, onClick }) {
  return (
    <motion.div
      ...
      onClick={onClick}
      className="relative max-w-sm rounded-xl overflow-hidden cursor-pointer shadow-md group"
    >
      {/* Imagen del lugar */}
      <motion.img
        src={imagen || "/default-image.jpg"} // Imagen por defecto si no hay
        alt={titulo}
        className="w-full h-56 object-cover brightness-90 transition-all duration-500 group-hover:brightness-100"
        whileHover={{ scale: 1.1 }}
      />

      {/* Overlay con degradado azul dinámico */}
      <motion.div
